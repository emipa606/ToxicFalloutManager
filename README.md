...
We're on Steam: https://steamcommunity.com/sharedfiles/filedetails/?id=2324530997
...
You'll add the mod. You'll enable the mod. It has no UI yet. It just works. If you don't want it, disable the mod.

The mod will add two new Allowed Areas: "ToxicH" and "ToxicA", for Humans and Animals, respectively. We'll come back to these.

When a Toxic Fallout happens, the mod will periodically check all your Colonists and Animals.

If their Toxic Exposure goes over 35%, the mod will change their Allowed Area to either "ToxicH" or "ToxicA", based on whether they are a Human or an Animal.

When their Toxic Exposure drops below 25%, the mod will set them back to whatever their previous Allowed Area was.

So your Area:"ToxicH" should be set to the majority of your base, under one large roof.

And your Area:"ToxicA" should be set to an animal area, under a roof, probably with Kibble available.

And this mod will then automatically manage sending your citizens to safety when their Toxic Exposure gets too high, and then let them go back outside for a while when it drops.
...
Unzip it, and it will spew out a single folder, which is probably named something like `ToxicFalloutManager-master`.
...
`C:\Program Files (x86)\Steam\steamapps\common\RimWorld\Mods\ToxicFalloutManager-master\Assemblies`
...
`C:\Program Files (x86)\Steam\steamapps\common\RimWorld\Mods\ToxicFalloutManager-master`
...
To do this, just click the button where it says "Branch: master", and then click the option for "beta". Congratulations! You've changed branches! Follow the same steps to download and install, except instead of `ToxicFalloutManager-master` it will now be `ToxicFalloutManager-beta`. You can have both versions installed, but please don't try to have both versions enabled at once using the in-game Mod menu.
